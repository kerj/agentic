...
// Backend is per-job; an 'auto'/unspecified job defaults to local (matches the
// server's _backend_of). Used only as the jobBackend() fallback for auto jobs.
function defaultBackend() { return 'local'; }
...
// Collapsed channel IDs — persisted across reloads
const _CH_COLLAPSE_KEY = 'agentic-ch-collapsed';
let chCollapsed = new Set(JSON.parse(localStorage.getItem(_CH_COLLAPSE_KEY) || '[]'));

function toggleChannelCollapse(cid) {
  if (chCollapsed.has(cid)) {
    chCollapsed.delete(cid);
  } else {
    // Don't collapse if the active thread lives in this channel
    if (chState.cid === cid) return;
    chCollapsed.add(cid);
  }
  localStorage.setItem(_CH_COLLAPSE_KEY, JSON.stringify([...chCollapsed]));
  loadChannels();
}
...
async function loadChannels() {
  ...
  try {
    ...
    chState.channels.forEach(ch => {
      ...
      // Never collapse a channel that contains the active thread
      const hasActive = chState.cid === ch.id;
      if (hasActive) chCollapsed.delete(ch.id);
      const collapsed = chCollapsed.has(ch.id);
      const chevron = collapsed ? '▶' : '▼';
      html += `<div class="ch-item ch-repo${collapsed?' collapsed':''}" title="${escAttr(ch.repo)}" onclick="toggleChannelCollapse('${escAttr(ch.id)}')">
        <span class="ch-repo-toggle">${chevron}</span>
        <span>📁 ${escHtml(shortRepo)}</span>
        <span class="ch-repo-path">${escHtml((ch.profile)||'')}</span></div>`;
      html += `<div class="ch-threads${collapsed?' collapsed':''}">`;
...
function transcriptBubble(e) {
  ...
  if (e.role === 'submitted') {
    ...
    const items = jobs.map(j => {
      const num = (j.seq != null) ? (j.seq + 1) : '';
      const title = escHtml(j.title || j.name || j.job_id || 'job');
      const link = j.job_id
        ? `<a class="ch-job-link" href="/job/${escAttr(j.job_id)}" title="Open this job">${escHtml(j.job_id)} ▸</a>`
        : '';
      const req = j.request ? `<div class="ch-job-req">${escHtml(j.request)}</div>` : '';
      return `<div class="ch-job-item">
        <div class="ch-job-head"><span class="ch-job-seq">#${num}</span> <span class="ch-job-title">${title}</span> ${link}</div>
        ${req}</div>`;
    }).join('');
    return `<details class="ch-bubble assistant ch-submitted">
      <summary>✓ Queued ${n} job(s)${n>1?' as a chain':''}</summary>
      <div class="ch-jobs">${items || '<div class="ch-job-empty">No job details recorded.</div>'}</div>
    </details>`;
  }
  return '';
}
...
function attachAskStream(es, live, tid) {
  ...
  es.onmessage = (m) => {
    ...
    if (t === 'tool_use') {
      ...
      c.scrollTop = c.scrollHeight;
    } else if (t === 'context_trimmed') {
      // Older turns were dropped to fit the planning context budget. Surface a
      // subtle muted note into the transcript (reusing the dim `ch-bubble tool`
      // system-bubble style) — inserted BEFORE the live answer bubble so it
      // reads in order. Does not disturb the in-flight answer_final render.
      const note = document.createElement('div');
      note.className = 'ch-bubble tool';
      note.textContent = '⚠ Older messages trimmed to fit context (' + (ev.dropped || 0) + ' dropped)';
      c.insertBefore(note, live);
      c.scrollTop = c.scrollHeight;
